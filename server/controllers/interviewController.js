...
function safeJsonParse(aiResponse) {
  try {
    ...
    // Extract JSON block if extra text is around
...
export const analyzeResume = async (req, res) => {
  try {
    ...
    const messages = [
      {
        ...
        content: `
          ...
          Do not include any text, labels, or code fences.
          {
            "role": "string",
            "experience": "string",
    ...
    console.log("Parsed projects type:", Array.isArray(parsed.projects), parsed.projects);
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    if (timeTaken > question.timelimit) {
      question.score = 0;
      question.feedback = "Time limit exceeded. Answer not allowec."
...
export const getMyInterviews = async (req,res) => {
  try {
    ...
    return res.status(400).json({ message: `Failed to find current user interview ${error}` })
  }
}
...
export const getInterviewReport = async (req, res) => {
  try {
    ...
    return res.json({
      finalScore: interview.finalScore, // corrected casing
      confidence: Number(avgConfidence.toFixed(1)),
      communication: Number(avgCommunication.toFixed(1)),
      correctness: Number(avgCorrectness.toFixed(1)),
    ...
    return res.status(400).json({ message: `Failed to find current user interview report ${error}` });
  }
};
